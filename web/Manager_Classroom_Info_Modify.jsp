...
<body>
	<div id="mainContent">
		<!-- 在这里编辑我的代码 -->
		<h2>修改教室信息:</h2>
		<form action="classroomEdit.do" method="post">
				...
				<label>教室所在大楼</label>
				 <select name="building" id="building" class="form-control">
						 ...
						 <c:forEach var="var" items="${applicationScope.buildings}" varStatus="status">
							 ...
							 <c:choose>
								 <c:when test="${var eq sessionScope.editClassroom.building}">
									 <option value="${sessionScope.editClassroom.building}"  selected>${sessionScope.editClassroom.building}</option>
								 </c:when>
								 <c:otherwise>
			...
			<div class="form-group">
				<label for="room_number">教室房间号</label> <input type="text"
					...
					value="${sessionScope.editClassroom.room_number}">
			</div>
